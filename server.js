...
app.get("/scores", (req, res) => {
  ...
  client.query(selectQuery, safeValue).then((result) => {
    ...
    const selectQueryR =
      "SELECT userresult FROM userProfile WHERE user_id=$1 ORDER BY userresult DESC ;";
...
async function handelRegister(request, res) {
  try {
    ...
    if (errors.length > 0) {
      ...
      const InsetIntoDataBaseQuery =
        "INSERT INTO users (name, email, pass , date) VALUES ($1, $2, $3, $4);";
...
function handleQuiz(req, res) {
  const { question, optionA, optionB, optionC, optionD, correctAnswer, email } =
    req.body;
  ...
  client.query(selectQuery, safeValue).then((result) => {
    ...
    const sqlQuery =
      "INSERT INTO quiz (user_id, question, optionA, optionB, optionC, optionD, correctAnswer) Values ($1, $2, $3, $4, $5, $6, $7);";
...
app.post("/submit", (req, res) => {
  ...
  client.query(selectQuery, safeValue).then((result) => {
    ...
    const sqlQuery =
      "INSERT INTO userProfile (user_id,userResult ) Values ($1, $2);";
...
function decodeHtml(str) {
  ...
  return str.replace(/&amp;|&lt;|&gt;|&quot;|&#039;|&pi;|&deg;/g, function (m) {
    return map[m];
  });
}
...
function Question(question) {
  ...
  this.choices = [
    decodeHtml(question.incorrect_answers[0]),
...
function updateQuiz(req, res) {
  ...
  const updateQuery =
    "UPDATE quiz SET question=$1, optionA=$2, optionB=$3, optionC=$4, optionD=$5,correctAnswer=$6 WHERE id=$7;";
